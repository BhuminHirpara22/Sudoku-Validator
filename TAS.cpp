...
atomic_flag Lock = ATOMIC_FLAG_INIT; // Lock for protecting the critical section
...
string timeTOString(system_clock::time_point time){
    ...
    ss << put_time(&local_tm, "%H:%M:%S") << "." 
       << setfill('0') << setw(6) << microsec.count(); 
...
void* getTask(void* arg){
    ...
    while(isValid){
        ...
        while(Lock.test_and_set(memory_order_acquire)) // Using test and set to allow only one thread to enter the critical section
            ;
        // If grid is invalid then early termination
        if(!isValid){
            Lock.clear(memory_order_release); // Free the lock
            break;
        }
