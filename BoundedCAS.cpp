...
atomic<int> current(-1);  // Current thread allowed to enter CS,-1 indicates no thread
vector<atomic<bool>> waiting;  // Array to track waiting threads
...
void* getTask(void* arg){
    ...
    while(isValid){
        ...
        waiting[data->id].store(true);

        int expected = -1;
        bool acquired = current.compare_exchange_strong(expected, data->id);
        // If the lock acquisition fails
        if(!acquired){
            // Busy-wait until the current thread's ID matches the lock holder's ID
            while((current.load() != data->id) && isValid)
                ;
        ...
        if(oldC>=totalTasks || actualTaskInc<=0){
            // Mark the current thread as no longer waiting to enter the critical section
            waiting[data->id].store(false);
            int next_id = -1;
            // Finds the next thread that is waiting to enter the critical section
            for(int offset=1;offset<=k;offset++){
                int candidate = (data->id+offset)%k;
                if(candidate<0 || candidate>=k) continue;
                if(waiting[candidate].load()){
                    next_id = candidate;
                    break;
                }
            }
            // If a waiting thread is found, pass the lock to that thread
            if(next_id != -1){
...
void readFile(){
    ...
    waiting = vector<atomic<bool>>(k); // Resize waiting vector based on the number of threads
    for(auto& w:waiting){
        w.store(false);
    }
    for (int i=0;i<n;i++){
